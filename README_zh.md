...
>- 另外，提供了用于人工核验的可执行程序，可以实时查看并修改图片文件名。
...
| `-w, --weights`                                | `Path` (`.pt`) |  ✅ | —                                         | Ultralytics YOLO 的权重文件        | 例如：`weights/best.pt`                                                              |
| `-o, --out-renamed`                            | `Path` (目录)    |  ✅ | —                                         | 重命名后图片的目标输出目录                 | **不能**与 `--input` 相同；若未指定 `--no-clean-out`，运行前会被清空                                |
| `--prompt`                                     | `str`          |  ✅ | —                                         | 发给 Ark 模型的 OCR 提示词            | 写清楚标签要识别的字段或格式                                                                    |
| `--duplicates`                                 | `True/False`   |  ✅ | —                                         | 重名处理策略                        | `True`：自动生成 `BASE-1/-2/...`；`False`：直接用 OCR 文本作为文件名（有冲突则跳过）                       |
| `--class-name`                                 | `str`          |  — | `"WhiteTag"`                              | YOLO 中要裁剪的类别名（会把所有检测合并成最小外接框） | 与模型的 `names` 大小写不敏感                                                               |
| `--ark-key`                                    | `str`          |  — | 环境变量 `ARK_API_KEY` → 代码中的兜底               | Ark 的 OCR API Key             | 优先级：命令行 `--ark-key` > 环境变量 `ARK_API_KEY` > 代码内默认值；**不建议明文写在代码里**                  |
| `--ark-model`                                  | `str`          |  — | `"doubao-1-5-thinking-vision-pro-250428"` | 使用的 Ark（豆包）视觉模型               | 必须是你的 Ark 端点已开通的模型                                                                |
| `--device`                                     | `str`          |  — | `"cpu"`                                   | 推理设备                          | 支持 `cpu`、`cuda`、`cuda:0`、`cuda:1`，或直接写数字表示 `cuda:<n>`                             |
...
| `--log-file`                                   | `Path`         |  — | 无（只打印到 stdout）                            | 除了 stdout 再额外写一份日志到文件         | 两种输出同时存在                                                                          |
